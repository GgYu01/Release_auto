...
class RepoManager:
    ...
    def parse_manifest(self, repo_config: RepoConfig):
        try:
            ...
            logger.error(f"Error parsing manifest for [bold blue]{repo_config.repo_name}[/]: {e}")
    ...
    def _parse_jiri_manifest(self, repo_config: RepoConfig):
        try:
            ...
            for project in root.findall("projects/project"):
                ...
                logger.info(f"Added GitRepoInfo for [bold blue]{repo_name}[/] from jiri manifest")
        except Exception as e:
            logger.error(f"Error parsing jiri manifest for [bold blue]{repo_config.repo_name}[/]: {e}")
    ...
    def _parse_repo_manifest(self, repo_config: RepoConfig):
        try:
            ...
            for project in root.findall("project"):
                ...
                repo_config.git_repos.append(git_repo_info)
                logger.info(f"Added GitRepoInfo for [bold blue]{repo_name}[/] from repo manifest")
        except Exception as e:
            logger.error(f"Error parsing repo manifest for [bold blue]{repo_config.repo_name}[/]: {e}")
        ...
        try:
            for repo_config in self.all_repos_config.repo_configs.values():
                if repo_config.repo_type == "git":
                    ...
                    repo_config.git_repos.append(git_repo_info)
                    logger.info(f"Added GitRepoInfo for [bold blue]{repo_config.repo_name}[/]")
